...
func build() error {
	err := goBuild("github.com/Cracked5pider/Havoc/teamserver/pkg/profile/yaotl/cmd/hcldec", "tmp_hcldec")
	if err != nil {
		return fmt.Errorf("error building hcldec: %s", err)
	}

	err = goBuild("github.com/Cracked5pider/Havoc/teamserver/pkg/profile/yaotl/cmd/hclspecsuite", "tmp_hclspecsuite")
	if err != nil {
		return fmt.Errorf("error building hcldec: %s", err)
	}
