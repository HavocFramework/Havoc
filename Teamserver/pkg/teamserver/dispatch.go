            ...
            for i := range t.Agents.Agents {
                if t.Agents.Agents[i].NameID == DemonID {
                    ...
                    if t.Agents.Agents[i].Info.MagicValue == agent.DEMON_MAGIC_VALUE {
                        ...
                        job, err = agent.TaskPrepare(pk.Body.Info["TaskID"].(string), command, pk.Body.Info)
                        if err != nil {
                        ...
                        if t.Agents.Agents[i].Pivots.Parent != nil {
                            ...
                            var (
                                Payload = agent.BuildPayloadMessage([]agent.DemonJob{job}, t.Agents.Agents[i].Encryption.AESKey, t.Agents.Agents[i].Encryption.AESIv)
                            ...
                            var PivotJob = agent.DemonJob{
                                Command: agent.COMMAND_PIVOT,
                                Data: []interface{}{
                                    agent.DEMON_PIVOT_SMB_COMMAND,
                                    DemonID32,
                                    Packer.Buffer(),
                                },
                            }
