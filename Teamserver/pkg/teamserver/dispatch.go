		...
		case packager.Type.Gate.Stageless:
			...
			if AgentType == "Demon" {
				go func() {
					...
					var StagelessPayload = builder.NewBuilder()

					StagelessPayload.ClientId = ClientID

					if StagelessPayload.ClientId == "" {
						logger.Error("Couldn't find the Client")
						return
					}

					StagelessPayload.SendConsoleMessage = SendConsoleMsg

					err = StagelessPayload.SetConfig(Config)
					if err != nil {
						return
					}

					if Arch == "x64" {
						StagelessPayload.SetArch(builder.ARCHITECTURE_X64)
					} else {
						StagelessPayload.SetArch(builder.ARCHITECTURE_X86)
					}

					var Ext string
					logger.Debug(Format)
					if Format == "Windows Exe" {
						StagelessPayload.SetFormat(builder.FILETYPE_WINDOWS_EXE)
						Ext = ".exe"
					} else if Format == "Windows Service Exe" {
						StagelessPayload.SetFormat(builder.FILETYPE_WINDOWS_SERVICE_EXE)
						Ext = ".exe"
					} else if Format == "Windows Dll" {
						StagelessPayload.SetFormat(builder.FILETYPE_WINDOWS_DLL)
						Ext = ".dll"
					} else if Format == "Windows Reflective Dll" {
						StagelessPayload.SetFormat(builder.FILETYPE_WINDOWS_REFLECTIVE_DLL)
						Ext = ".dll"
					} else if Format == "Windows Shellcode" {
						StagelessPayload.SetFormat(builder.FILETYPE_WINDOWS_RAW_BINARY)
						Ext = ".bin"
					} else {
					...
					for i := 0; i < len(t.Listeners); i++ {
						if t.Listeners[i].Name == ListenerName {
							StagelessPayload.SetListener(t.Listeners[i].Type, t.Listeners[i].Config)
						}
					}

					StagelessPayload.SetOutputPath("/tmp/" + utils.GenerateID(10) + Ext)

					if t.Profile.Config.Demon != nil {
						if t.Profile.Config.Demon.Binary != nil {
							StagelessPayload.SetPatchConfig(t.Profile.Config.Demon.Binary.Header)
						}
					}

					if StagelessPayload.Build() {
						pal := StagelessPayload.GetPayloadBytes()
						if len(pal) > 0 {
							err := t.SendEvent(StagelessPayload.ClientId, events.Gate.SendStageless("demon"+Ext, pal))
							if err != nil {
