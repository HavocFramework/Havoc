...
void HavocNamespace::UserInterface::Widgets::SessionTable::NewSessionItem( Util::SessionItem item ) const
{
    ...
    for ( auto & Session : HavocX::Teamserver.Sessions )
    {
        ...
        if ( Session.InteractedWidget == nullptr )
        {
            ...
            AgentMessageInfo =
                    Util::ColorText::Comment( "[" + item.First + "]" ) + " Agent " + Util::ColorText::Red( item.Name.toUpper() ) + " authenticated from " + Util::ColorText::Cyan( item.External ) + " as "+ Util::ColorText::Purple( item.Computer + "\\" + item.User ) +
